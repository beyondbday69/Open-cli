...
type messagesCmp struct {
	...
	// atBottom tracks whether the viewport was scrolled to the bottom before
	// the last update. Only auto-scroll on new content when true.
	atBottom bool
}
...
func (m *messagesCmp) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		if key.Matches(msg, messageKeys.PageUp) || key.Matches(msg, messageKeys.PageDown) ||
			...
			key.Matches(msg, key.NewBinding(key.WithKeys("up", "down"))) {
			u, cmd := m.viewport.Update(msg)
			m.viewport = u
			cmds = append(cmds, cmd)
			// Track whether user is now at the bottom after scrolling
	...
	case pubsub.Event[message.Message]:
		...
		if needsRerender {
			...
			if len(m.messages) > 0 {
				...
				// This lets users scroll up to read history without being snapped back.
				if m.atBottom || msg.Type == pubsub.CreatedEvent && m.viewport.TotalLineCount() == 0 {
					m.viewport.GotoBottom()
					m.atBottom = true
				}
			}
