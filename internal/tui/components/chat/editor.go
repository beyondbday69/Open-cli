...
func (m *editorCmp) send() tea.Cmd {
	...
	if strings.HasPrefix(value, "/") {
		cmd := parseSlashCommand(value)
		if cmd != "" {
			m.textarea.Reset()
			return util.CmdHandler(SlashCommandMsg{Command: cmd})
		}
		// unknown slash command — show hint, don't clear
		return util.ReportWarn("Unknown command. Try: /model /session /theme /file /help /logs /new /commands")
	}
...
// parseSlashCommand returns the canonical command name for a slash input,
// or "" if the input doesn't match any known command.
func parseSlashCommand(input string) string {
	// strip leading slash and any arguments
...
func (m *editorCmp) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case dialog.CompletionSelectedMsg:
		// If the completion is a slash command, execute it immediately.
		if strings.HasPrefix(msg.CompletionValue, "/") {
	...
	// Check for completions triggers
	if keyMsg, ok := msg.(tea.KeyMsg); ok && m.textarea.Focused() {
		value := m.textarea.Value()
		if value == "/" && keyMsg.String() == "/" {
			return m, tea.Batch(cmd, func() tea.Msg {
				return dialog.ShowCompletionDialogMsg{
					Provider: completions.NewSlashCommandContextGroup(),
				}
			}, func() tea.Msg {
				return tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune{'/'}}
			})
		} else if (strings.HasSuffix(value, " @") || value == "@") && keyMsg.String() == "@" {
			return m, tea.Batch(cmd, func() tea.Msg {
				return dialog.ShowCompletionDialogMsg{
					Provider: completions.NewFileAndFolderContextGroup(),
				}
			}, func() tea.Msg {
				return tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune{'@'}}
			})
