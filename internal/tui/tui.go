...
func (a appModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case dialog.ShowCompletionDialogMsg:
		a.completionDialog = dialog.NewCompletionDialogCmp(msg.Provider)
		a.showCompletionDialog = true
		return a, a.completionDialog.Init()
	...
	if a.showCompletionDialog {
		d, compCmd := a.completionDialog.Update(msg)
		a.completionDialog = d.(dialog.CompletionDialog)
...
// handleSlashCommand performs the same action as the corresponding Ctrl key.
func (a *appModel) handleSlashCommand(cmd string) (tea.Model, tea.Cmd) {
	// Only act when on the chat page and no modal is already open
	onChat := a.currentPage == page.ChatPage
	noModal := !a.showQuit && !a.showPermissions

	switch cmd {
	case "model":
		if onChat && noModal && !a.showSessionDialog && !a.showCommandDialog {
			a.showModelDialog = !a.showModelDialog
		}
	case "session":
		if onChat && noModal && !a.showCommandDialog {
			sessions, err := a.app.Sessions.List(context.Background())
			if err != nil {
				return a, util.ReportError(err)
			}
	...
	case "theme":
		if noModal && !a.showSessionDialog && !a.showCommandDialog {
			a.showThemeDialog = true
			return a, a.themeDialog.Init()
		}
	case "file":
	...
	case "commands":
		if onChat && noModal && !a.showSessionDialog && !a.showThemeDialog && !a.showFilepicker {
			if len(a.commands) == 0 {
				return a, util.ReportWarn("No commands available")
			}
			a.commandDialog.SetCommands(a.commands)
